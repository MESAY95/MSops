...
// const MaterialForm = ({ open, onClose, material, viewMode, onSave, loading }) => {
...
//   useEffect(() => {
//     if (material) {
//       // Parse material name to extract material code if it exists
//       const materialName = material.Material || '';
//       let extractedCode = '';
//       let displayName = materialName;
      
//       // Extract code from format: "Material Name / [CODE]"
//       const codeMatch = materialName.match(/\/\s*\[([^\]]+)\]$/);
//       if (codeMatch) {
//         extractedCode = codeMatch[1];
//         displayName = materialName.replace(/\s*\/\s*\[[^\]]+\]\s*$/, '').trim();
//       }
...
//     try {
//       // Check if any existing material has the same code in its name
//       const combinedName = `${formData.Material.trim()} / [${code.trim()}]`;
...
//         // Check if material name contains the same code pattern
//         const existingCodeMatch = existingMaterial.Material.match(/\/\s*\[([^\]]+)\]$/);
...
//   // Validation rules based on Material model
//   const validateField = async (name, value) => {
//     const newErrors = { ...errors };

//     switch (name) {
//       case 'Material':
//         if (!value.trim()) newErrors.Material = 'Material name is required';
//         else if (value.trim().length < 2) newErrors.Material = 'Material name must be at least 2 characters';
//         else if (value.trim().length > 100) newErrors.Material = 'Material name must be less than 100 characters';
//         else delete newErrors.Material;
//         break;
...
//           newErrors.MaterialCode = 'Material code must be less than 10 characters';
//         } else if (!/^[A-Z0-9]+$/.test(value)) {
//           newErrors.MaterialCode = 'Material code can only contain uppercase letters and numbers';
//         } else {
...
//         else if (parseInt(value) > 104) newErrors.MinimumLeadTime = 'Minimum lead time cannot exceed 104 weeks';
...
//         else if (parseInt(value) > 104) newErrors.MaximumLeadTime = 'Maximum lead time cannot exceed 104 weeks';
...
//     // Validate the field if it's been touched
//     if (touched.MaterialCode) {
//       validateField('MaterialCode', value);
//     }
//   };
...
//       validateField(field, materialCode);
//     } else {
...
//     const allTouched = {};
//     [...Object.keys(formData), 'MaterialCode'].forEach(key => {
//       allTouched[key] = true;
//     });
...
//     }
//     await validateField('MaterialCode', materialCode);

//     // Final duplicate check before submitting
//     if (materialCode) {
//       const isDuplicate = await checkDuplicateMaterialCode(materialCode);
//       if (isDuplicate) {
//         setErrors(prev => ({
//           ...prev,
...
//     if (isFormValid()) {
//       // Combine material name and code in the required format
//       const combinedMaterialName = `${formData.Material.trim()} / [${materialCode.trim()}]`;
...
//                     {formData.Material && materialCode 
//                       ? `${formData.Material} / [${materialCode}]`
...
//                 <Box sx={{ mt: 1 }}>
//                   <Chip 
//                     label={`Range: ${leadTimeRange} weeks`}
//                     variant="outlined"
...
//               • Material name and code will be combined as: <strong>Material Name / [CODE]</strong><br/>
//               • Example: <strong>Cement / [CEM]</strong> or <strong>Steel Rods / [STR]</strong><br/>
//               • Material code will be automatically converted to uppercase<br/>
//               • <strong>Duplicate material codes are not allowed</strong>
//             </Typography>
//           </Box>
...
//               Material Code Duplication is Restricted!!!
//             </Alert>
//           )}
//         </DialogContent>
